...
BASE_LOG = os.path.join(LOGDIR, "zombie-diplo.log")
_DEFAULT_LOG_LEVEl = logging.INFO
_CONFIG_ENV = "zombie_diplo"
...
def _determine_log_level_or_default() -> int:
    ...
    :return:
    """
    innerlog = get_logger("ZombieDiploLoggingConfig", _use_default_log_level=True)
    maybe_config_file = os.getenv(_CONFIG_ENV)
    if maybe_config_file and os.path.exists(maybe_config_file):
        ...
        innerlog.error(
            ...
            f"by environment variable {_CONFIG_ENV} ; using default logging level.  "
